...
if (x_val>=0)
    s=2;%given that part of the above if condiiton falls within the stability region, 
    % we have to consider a better option for when real(lambda)>=0)
    betaS = (s+4)*(s-1)/3;
    ...
    %dt until a desired dt with the given lambda ensures stability. 
    %Afterwards spit out the new dt value and the given eigenvalue for that
...
%beta is not negative). This region is independent of t he number of stages
