//to run this program on MACOS compile using the command below and run the executable dummynam
// gcc-15 -fopenmp <filename>  -o <dummyname>
...
int main(int argc, char *argv[]) {
   ...
   //below is the parallelised version of the while loop: using Tim's solution not mine
   int count = 0;
   start = omp_get_wtime();
   while (p != NULL) {
      p = p->next;
