...
function [x_sol, r, num_iter] = UnPreconBCGS(x0,s1, s2, A_up, A_low, A_diag, delta, max_iter)
...
for k = 0:max_iter
    Ap = A*p;
    alpha = dot(rstar, Ap); %original algorithm is dot(rstar, Ap)
    ...
    rho = sqrt(dot(r,r));
    residual_norm = norm(b - A*x_sol)/norm(b);
    num_iter = k;

    if (rho<=delta)
    % if (residual_norm<=delta)
        fprintf("Algorithm converged at %d iterations with relative residual = %0.16e \n", k, residual_norm);
        break;
    end
    

    beta_num = dot(rstar, r); %original algorithm is dot(rstar, r)
    beta = (beta_num / beta_denom)*(alpha / omega);
    %p = r + beta*(p - omega*Ap); 
    p = beta*p - alpha*(beta_num/beta_denom)*Ap + r;
    beta_denom = beta_num;
    % k = k + 1;
end
