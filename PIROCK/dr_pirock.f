...
c			include 'pb_bruss2dreac.f' 
c      parameter (nsd=200,npdes=2,neqn=nsd*nsd*npdes)
c
			include 'pb_bruss2dadv.f' 
      parameter (nsd=400,npdes=0,neqn=nsd*2)

c      include 'pb_bruss1dadv.f' 
c      parameter (nsd=400,npdes=0,neqn=nsd*2)
c
c      include 'pb_bruss1dadv3V.f' 
c      parameter (nsd=512,npdes=0,neqn=nsd*3)
c
c			include 'pb_pides.f' 
c      parameter (nsd=100,npdes=0,neqn=nsd)
c
c			include 'pb_bruss2d.f' 
c      parameter (nsd=200,npdes=2,neqn=nsd*nsd*npdes)
c
c			include 'pb_comb.f' 
c      parameter (nsd=100,npdes=2,neqn=nsd*nsd*npdes)
c ----------------------------------------------------
			implicit double precision (a-h,o-z)
			CHARACTER(len=32) :: arg
      external fd,fa,fr,fw
c ----- to integrate with rock2.f 
      dimension y(neqn),work(15*neqn),frjac(neqn*npdes) 
      integer iwork(25),idid,ijac(neqn)
c for combustion problem
      double precision K0
...
c     for 'pb_bruss2dreac.f' or  'pb_comb.f'
c      iwork(2)=0
c			iwork(20)=0
c			iwork(21)=1
c			iwork(22)=0
			
c     for 'pb_bruss2dadv.f'
			iwork(20)=1
			iwork(21)=0
			iwork(22)=0
...
c ----- integration -----
			...
			CALL CPU_TIME(time0)
c ----- to integrate with rock2.f   
      call irock2(neqn,npdes,t,tend,h,y,fd,fa,fr,fw,atol,rtol,
